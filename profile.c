...
#define KEY_CPU_NO_TURBO "CpuNoTurbo"
...
char *read_string(GKeyFile *file, const char *group, const char *key){
    gsize length;
    GError *error = NULL;
...
int read_int(GKeyFile *file, const char *group, const char *key){
    gsize length;
    GError *error = NULL;
...
int read_cpu_max_freq(Profile_t *profile, GKeyFile *file){
    profile->cpu_max_freq = read_int(file,GROUP_POWER,KEY_CPU_MAX_FREQ);
    return profile->cpu_max_freq<0;
}

int read_cpu_min_freq(Profile_t *profile, GKeyFile *file){
    profile->cpu_min_freq = read_int(file,GROUP_POWER,KEY_CPU_MIN_FREQ);
    return profile->cpu_min_freq<0;
}

int read_cpu_min_perf(Profile_t *profile, GKeyFile *file){
    profile->cpu_min_perf = read_int(file,GROUP_POWER,KEY_CPU_MIN_PERF);
    return profile->cpu_min_perf<0;
}

int read_cpu_max_perf(Profile_t *profile, GKeyFile *file){
    profile->cpu_max_perf = read_int(file,GROUP_POWER,KEY_CPU_MAX_PERF);
    return profile->cpu_max_perf<0;
}

int read_cpu_no_turbo(Profile_t *profile, GKeyFile *file){
    GError *error = NULL;
    gboolean result = g_key_file_get_boolean(file,GROUP_POWER,KEY_CPU_NO_TURBO,&error);
    if(error){
        printf("%s\n",error->message);
        return 1;
    }
    profile->cpu_no_turbo = result;
    return 0;
}

int read_cpu_scaling_governor(Profile_t *profile, GKeyFile *file){
    profile->cpu_governor = read_string(file,GROUP_POWER,KEY_CPU_SCALING_GOVERNOR);
...
int read_cpu_energy_pref(Profile_t *profile, GKeyFile *file){
    profile->cpu_energy_pref = read_string(file,GROUP_POWER,KEY_CPU_ENERGY_PREFERENCE);
...
Profile_t *open_profile(const char *filename){
    ...
    error |= read_cpu_no_turbo(profile,file);
